      ...
      Layout({
        ...
        children: html`
          <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-text-primary">Zeiteintr&auml;ge</h1>
            <a href="/zeiten/new" class="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
              + Neuer Zeiteintrag
            </a>
          </div>
          ${
    ...
    <div class="max-w-2xl">
      ...
      <form method="post" action="${action}" class="space-y-6 rounded-lg border border-border-subtle bg-bg-surface shadow-card p-6">
        <div>
          ...
          <select id="project_id" name="project_id" required class="${INPUT_CLASS}">
            <option value="">-- Wählen --</option>
            ${allProjects.map((p) => html`<option value="${p.id}" ${entry?.project_id === p.id ? "selected" : ""}>${p.client_name} / ${p.name} (${p.code})</option>`)}
          </select>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label for="date" class="block text-sm font-medium text-text-primary">Datum *</label>
            <input type="date" id="date" name="date" required value="${entry?.date || ""}" class="${INPUT_CLASS}" />
          </div>
          <div>
            <label for="duration" class="block text-sm font-medium text-text-primary">Dauer *</label>
            <input type="number" id="duration" name="duration" required min="0.25" max="24" step="0.25" value="${entry?.duration || ""}" class="${INPUT_CLASS}" aria-describedby="duration-hint" />
            <p id="duration-hint" class="mt-1 text-xs text-text-muted">Dauer in Stunden (z.B. 8 für einen ganzen Tag).</p>
          </div>
        </div>
        <div>
          <label for="description" class="block text-sm font-medium text-text-primary">Beschreibung</label>
          <textarea id="description" name="description" rows="3" class="${INPUT_CLASS}" aria-describedby="description-hint">
${entry?.description || ""}</textarea>
        ...
        <div>
          <div class="flex items-center">
            <input type="checkbox" id="billable" name="billable" ${entry?.billable === 1 ? "checked" : ""} class="h-4 w-4 rounded border-border-medium accent-primary" aria-describedby="billable-hint" />
